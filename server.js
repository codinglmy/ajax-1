...
var server = http.createServer(function (request, response) {
  ...
  if (path === "/") {
    ...
    const li = arr.map((item) => `<li>${item.id}</li>`).join(" ");
    html = html.replace("{{page1}}", `<ul id="list">${li}</ul>`);
    response.write(html);
    response.end();
  } else if (path === "/x") {
    response.statusCode = 200;
    ...
    response.statusCode = 200;
    response.setHeader("Content-Type", "text/html;charset=utf-8");
    const html = fs.readFileSync("public/3.html");
    response.write(html);
    response.end();
  } else if (path === "/4.xml") {
    response.statusCode = 200;
    response.setHeader("Content-Type", "text/xml;charset=utf-8");
    const xml = fs.readFileSync("public/4.xml");
    response.write(xml);
    response.end();
  } else if (path === "/5.json") {
    response.statusCode = 200;
